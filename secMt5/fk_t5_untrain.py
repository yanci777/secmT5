...
def train_training(train_data, tokenizer, epoch_i, model):
    ...
    for step, batch in enumerate(train_data):
        ...
        # 以下是无法回答类型loss计算
        # 对问题注意力结果进行激活
        pNA = torch.sigmoid(ques2context_att)
        # 再连接一个全连接层 将 batch * ques_len * dim -> batch * dim -> batch * 2
        nor_UN = fkUtils.weighted_sum(pNA, 768, device)
        linear = torch.nn.Linear(768, 2)
        sm_UN = torch.softmax(linear(nor_UN), dim=1)
        print(sm_UN)
        # 初始化标签
        if hasattr(batch, 'impossible'):
            label_UN = torch.Tensor([1]).long()
        else:
            label_UN = torch.Tensor([0]).long()
        print(label_UN)
        # 计算无法回答loss
        cross_UN = torch.nn.CrossEntropyLoss()
        loss_UN = cross_UN(sm_UN, label_UN)
        print(loss_UN)
        print(loss)
...
if __name__ == '__main__':
    ...
    tokenizer = T5PegasusTokenizer.from_pretrained(model_path, vocab_file='dataset/vocab_pro.txt')
